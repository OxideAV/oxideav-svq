...
fn main() {
    ...
    emit_svq3_tables(&tables_dir, &out_dir);
}

/// The SVQ3 data tables mirrored bit-exact from `docs/video/svq3/tables/`
/// (SHA-256s in `tables/MANIFEST-svq3.sha256`), emitted into
/// `$OUT_DIR/svq3_tables_data.rs` and included by `src/svq3_tables.rs`:
///
/// * `svq3-07-intra4x4-pred-mode-pairs.csv` — the 25 `(rank_first,
///   rank_second)` records one intra 4×4 prediction-mode pair code
///   selects (spec/07 §6 item 1);
/// * `svq3-08-intra4x4-pred-mode-context.csv` — the `6 × 6 × 5`
///   neighbour-context table resolving a rank into a prediction mode
///   (spec/07 §10.2), `9` marking an illegal `(context, rank)`;
/// * `svq3-09-intra-picture-edge-filter-limit.csv` — the 32-entry
///   per-quantiser limit of the intra-picture edge filter (spec/09 §2).
fn emit_svq3_tables(tables_dir: &Path, out_dir: &Path) {
    let pairs_csv = tables_dir.join("svq3-07-intra4x4-pred-mode-pairs.csv");
    let context_csv = tables_dir.join("svq3-08-intra4x4-pred-mode-context.csv");
    let limit_csv = tables_dir.join("svq3-09-intra-picture-edge-filter-limit.csv");
    for p in [&pairs_csv, &context_csv, &limit_csv] {
        println!("cargo:rerun-if-changed={}", p.display());
    }

    let pairs = parse_csv_rows(&pairs_csv, "code,rank_first,rank_second", 3);
    assert_eq!(
        pairs.len(),
        25,
        "{}: expected 25 pair records",
        pairs_csv.display()
    );
    for (i, row) in pairs.iter().enumerate() {
        assert_eq!(
            row[0],
            i as i64,
            "{}: code column must run 0..25",
            pairs_csv.display()
        );
        assert!((0..=4).contains(&row[1]) && (0..=4).contains(&row[2]));
    }

    let context = parse_csv_rows(&context_csv, "top_context,left_context,rank,pred_mode", 4);
    assert_eq!(
        context.len(),
        180,
        "{}: expected 180 rows",
    ...
    let mut context_table = [[[u8::MAX; 5]; 6]; 6];
    for (i, row) in context.iter().enumerate() {
        let (top, left, rank, mode) = (row[0], row[1], row[2], row[3]);
        assert_eq!(
            (top, left, rank),
            ((i / 30) as i64, ((i / 5) % 6) as i64, (i % 5) as i64),
            "{}: rows must be top-major, left-minor, rank-inner",
            context_csv.display()
        );
        assert!(
            (0..=4).contains(&mode) || mode == 9,
            "{}: bad mode {mode}",
            context_csv.display()
        );
        context_table[top as usize][left as usize][rank as usize] = mode as u8;
    }

    let limit = parse_csv_rows(&limit_csv, "quantiser_index,edge_filter_limit", 2);
    assert_eq!(limit.len(), 32, "{}: expected 32 rows", limit_csv.display());
    for (i, row) in limit.iter().enumerate() {
        assert_eq!(row[0], i as i64);
        assert!((0..=255).contains(&row[1]));
    }

    let out_path = out_dir.join("svq3_tables_data.rs");
    let mut f = fs::File::create(&out_path)
        .unwrap_or_else(|e| panic!("create {}: {e}", out_path.display()));
    writeln!(
        f,
        "// AUTO-GENERATED by build.rs from tables/svq3-07..09 (mirrored from\n\
         // docs/video/svq3/tables/). DO NOT EDIT — re-run cargo build to regenerate."
    )
    .unwrap();

    writeln!(
        f,
        "\n/// tables/07: `(rank_first, rank_second)` per pair code 0..=24.\n\
         pub static SVQ3_INTRA4X4_PRED_MODE_PAIRS: [(u8, u8); 25] = ["
    )
    .unwrap();
    for row in &pairs {
        writeln!(f, "    ({}, {}),", row[1], row[2]).unwrap();
    }
    writeln!(f, "];").unwrap();

    writeln!(
        f,
        "\n/// tables/08: `[top_context][left_context][rank]` → prediction mode (9 = illegal).\n\
         pub static SVQ3_INTRA4X4_PRED_MODE_CONTEXT: [[[u8; 5]; 6]; 6] = ["
    )
    .unwrap();
    for top in &context_table {
        writeln!(f, "    [").unwrap();
        for left in top {
            writeln!(
                f,
    ...
    writeln!(
        f,
        "\n/// tables/09: the edge-filter limit per quantiser index 0..=31.\n\
         pub static SVQ3_INTRA_EDGE_FILTER_LIMIT: [u8; 32] = ["
    )
    .unwrap();
    let mut buf = String::new();
    for row in &limit {
        buf.push_str(&format!("{}, ", row[1]));
    }
    writeln!(f, "    {}", buf.trim_end()).unwrap();
    writeln!(f, "];").unwrap();
}

/// Parse a plain integer CSV whose first line is exactly `header` and
/// whose every other non-empty line has `columns` integer fields.
fn parse_csv_rows(path: &Path, header: &str, columns: usize) -> Vec<Vec<i64>> {
    let text = fs::read_to_string(path).unwrap_or_else(|e| panic!("read {}: {e}", path.display()));
    let mut rows = Vec::new();
    for (lineno, line) in text.lines().enumerate() {
        let line = line.trim();
        if lineno == 0 {
            assert_eq!(line, header, "{}: unexpected header", path.display());
            continue;
        }
        if line.is_empty() {
            continue;
        ...
        let row: Vec<i64> = line
            .split(',')
            .map(|c| {
                c.trim().parse::<i64>().unwrap_or_else(|e| {
                    panic!("{}:{}: parse {c:?}: {e}", path.display(), lineno + 1)
                })
            })
            .collect();
        assert_eq!(
